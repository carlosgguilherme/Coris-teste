...
filtro de período (30 dias, 90 dias, 12 e 24 meses) e quatro visões: visão geral, marketing,
comercial e sinistros/atendimento. O período e a visão ficam na URL, o que permite compartilhar o link.

\subsection{Tabelas}

As tabelas antigas não foram alteradas diretamente: foram criadas \emph{migrations} novas.
...
Tabela & Conteúdo \\
\midrule
\texttt{canais} & site, agências, corretores, parceiros e app \\
\texttt{campanhas} & campanhas de marketing, com UTM, orçamento e investimento \\
\texttt{cotacoes} & toda simulação de preço, convertida em apólice ou abandonada \\
\texttt{funil\_eventos} & um registro para cada etapa que o cliente passou na cotação \\
\texttt{sinistros} & ocorrências cobertas pela apólice: valor reclamado, valor pago e status \\
\texttt{atendimentos} & contatos com a central 24h: canal, tempo de espera, SLA e nota NPS \\
\bottomrule
\end{tabularx}
\end{center}

A apólice ganhou \texttt{canal\_id} e \texttt{campanha\_id}, opcionais, para saber de onde veio cada venda.
...
São 37 testes automatizados (PHPUnit), executados com \texttt{php artisan test} na pasta
\arq{backend}.
...
\begin{itemize}
  ...
  \item \textbf{Da dashboard:} \arq{DashboardApiTest} verifica os indicadores de cada visão, o funil,
        o ROI de campanha, a sinistralidade e o seeder de demonstração.
\end{itemize}
